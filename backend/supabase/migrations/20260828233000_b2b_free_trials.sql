-- B2B Free Trial v1
-- Keep backend and frontend Supabase migration trees aligned.
...
CREATE INDEX IF NOT EXISTS pending_free_trial_email_idx ON public.pending_free_trial_registrations (lower(email));
...
CREATE TABLE IF NOT EXISTS public.free_trial_workspaces (
  ...
  country text NOT NULL,
  language text NOT NULL CHECK (language IN ('fi', 'en', 'sv')),
  status text NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'expired', 'ended', 'converted')),
  ...
  engagement_category text NOT NULL DEFAULT 'low' CHECK (engagement_category IN ('low', 'medium', 'high')),
  utm_source text,
  utm_medium text,
  utm_campaign text,
  utm_content text,
...
CREATE INDEX IF NOT EXISTS free_trial_status_end_idx ON public.free_trial_workspaces (status, trial_ends_at);
CREATE INDEX IF NOT EXISTS free_trial_creator_idx ON public.free_trial_workspaces (creator_user_id);
...
CREATE TABLE IF NOT EXISTS public.free_trial_members (
  ...
  membership_kind text NOT NULL DEFAULT 'creator' CHECK (membership_kind IN ('creator', 'school_admin_invite')),
...
CREATE INDEX IF NOT EXISTS free_trial_referrer_idx ON public.free_trial_referrals (referrer_trial_id);
...
CREATE INDEX IF NOT EXISTS free_trial_events_timeline_idx ON public.free_trial_events (trial_id, created_at DESC);
...
BEGIN
  ...
  UPDATE public.schools s SET is_active = false
  FROM public.free_trial_workspaces t
  WHERE t.school_id = s.id AND t.status IN ('expired', 'ended') AND s.account_kind = 'trial';
