 ...
 * Returns an opaque one-time token that is placed in auth user metadata. The
 * token is consumed only after the email address has been confirmed.
 */
...
export const finalizeStaffRegistration = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: pending, error: pendingError } = await db
      ...
      .select("id, email, display_name, school_id, language, expires_at")
    ...
    const { error: profileError } = await db.from("profiles").upsert(
      {
        ...
        school_id: school.id,
        language: school.language ?? pending.language ?? "fi",
      },
      { onConflict: "id" },
    );
    ...
    return {
      ok: true as const,
      schoolName: school.name as string,
      language: (school.language ?? pending.language ?? "fi") as string,
    };
  });
