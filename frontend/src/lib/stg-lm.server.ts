...
function systemPrompt(request: StgLmRequest) {
  const rules = [
  ...
  if (request.task === "reflection_followup") {
    rules.push('Return ONLY JSON: {"compliment":"...","question":"..."}.');
    ...
    rules.push('Return ONLY JSON: {"thanks":"..."}.');
...
function userPrompt(request: StgLmRequest) {
  const lines = [
...
export async function runStgLm(request: StgLmRequest): Promise<StgLmResponse> {
  ...
  if (!live) {
    return request.task === "reflection_followup"
      ? { ...fallbackFollowup(request.language), model: "STG-LM-1", mode }
      : { ...fallbackClose(request.language), model: "STG-LM-1", mode };
  }
  ...
  try {
    ...
    if (request.task === "reflection_followup") {
      ...
      if (compliment && question) return { compliment, question, model: "STG-LM-1", mode };
      return { ...fallbackFollowup(request.language), model: "STG-LM-1", mode };
    }

    const thanks = String(parsed?.thanks ?? "").trim();
    if (thanks) return { thanks, model: "STG-LM-1", mode };
    return { ...fallbackClose(request.language), model: "STG-LM-1", mode };
  } catch (error) {
