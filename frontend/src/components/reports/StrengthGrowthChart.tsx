...
/** Compact range control used by the student chart. */
...
export interface StrengthGrowthChartProps {
  events: ReportEvent[];
  lang: "fi" | "en" | "sv";
  /** Controlled range; when omitted the chart keeps its own state. */
  selectedRange?: RangeDays;
  onRangeChange?: (d: RangeDays) => void;
  showRangeControls?: boolean;
  showShowAllButton?: boolean;
  /** "all" always draws every collected strength; "top10" is the report default. */
  visibleStrengthMode?: "all" | "top10";
  emptyLabel?: string;
  seed?: string;
  printMode?: boolean;
  /** @lovable-new 2026-08-05 — avoids flashing the empty state while loading. */
  loading?: boolean;
}
...
export function StrengthGrowthChart({
  ...
  // @lovable-new 2026-08-05 — explicit locale per application language.
...
/**
 * @lovable-new 2026-08-05 — Tooltip resolves each series back to its strength
 * name and adds a translated cumulative total row at the bottom.
 */
