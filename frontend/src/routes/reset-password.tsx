...
function ResetPasswordPage() {
  ...
  const { source, lang } = Route.useSearch();

  const [detectedSuperAdmin, setDetectedSuperAdmin] = useState(
    source === "superadmin",
  );

  const isSuperAdmin =
    source === "superadmin" || detectedSuperAdmin;

  const loginTo = isSuperAdmin
    ? "/superadmin/login"
    : "/auth/login";
  ...
  /*
   * Keep the reset page in the same language that was used
   * when the password-reset email was requested.
   *
   * Example:
   * /reset-password?lang=fi
   * -> Finnish reset page.
   */
  useEffect(() => {
    if (lang) {
