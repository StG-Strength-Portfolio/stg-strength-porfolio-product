-- Follow-up hardening for school-admin lifecycle actions.
-- Students inherit Customer scope through their active/deleted class membership,
-- while promoted school admins may still own classes. Handle both consistently.
...
BEGIN
  ...
  v_target_school := public.user_school_id(p_user_id);
  IF v_target_role IS NULL OR v_target_school IS NULL OR v_target_school <> v_actor_school THEN
  ...
  IF p_user_id = v_actor AND v_action IN ('deactivate', 'delete', 'demote_to_teacher') THEN
    RAISE EXCEPTION 'You cannot remove your own active school-admin access';
  END IF;
  ...
  -- Teachers and promoted school admins can own classes. Before deactivation or
  -- deletion, transfer every active owned class to another active same-school teacher.
  IF v_target_role IN ('teacher', 'school_admin') AND v_action IN ('deactivate', 'delete') THEN
    ...
    IF v_owned_count > 0 THEN
      ...
      IF EXISTS (
        ...
        WHERE p.id = p_replacement_teacher_id
          AND (p.deleted_at IS NOT NULL OR p.deactivated_at IS NOT NULL OR COALESCE(p.locked, false) = true)
      ) THEN
