...
-- A trial teacher must explicitly confirm school authorization before the first
-- real class can be created. Paid-school behavior is unchanged.
...
AS $$
DECLARE
  v_trial public.free_trial_workspaces%ROWTYPE;
BEGIN
  SELECT t.* INTO v_trial
  FROM public.profiles p
  JOIN public.free_trial_workspaces t ON t.school_id = p.school_id
  WHERE p.id = NEW.teacher_id
  LIMIT 1;

  IF FOUND THEN
    IF v_trial.status <> 'active' OR v_trial.trial_ends_at <= now() THEN
      RAISE EXCEPTION 'free_trial_expired';
    END IF;
    IF v_trial.authorization_confirmed_at IS NULL THEN
      RAISE EXCEPTION 'free_trial_school_authorization_required';
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trg_enforce_trial_class_authorization
  BEFORE INSERT ON public.classes
  FOR EACH ROW EXECUTE FUNCTION public.enforce_trial_class_authorization();

-- Track students who enter via a class belonging to a trial workspace. This is
-- metadata only; it never copies student portfolio/response content.
...
BEGIN
  ...
  IF v_trial_id IS NOT NULL THEN
    INSERT INTO public.free_trial_students (trial_id, user_id, class_id)
    VALUES (v_trial_id, NEW.student_id, NEW.class_id)
    ON CONFLICT DO NOTHING;

    INSERT INTO public.free_trial_events (trial_id, user_id, event_name, event_properties)
    VALUES (v_trial_id, NEW.student_id, 'student_joined', jsonb_build_object('class_id', NEW.class_id));
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER trg_track_free_trial_student
  AFTER INSERT ON public.class_members
  FOR EACH ROW EXECUTE FUNCTION public.track_free_trial_student();

-- A Principal/School Admin trial can have at most five invited teachers.
-- The creator is not counted as an invited teacher. Existing paid-school staff
...
BEGIN
  ...
  IF v_trial.creator_role = 'school_admin' THEN
    ...
    IF v_teacher_count >= 5 THEN
      RAISE EXCEPTION 'free_trial_teacher_invite_limit_reached';
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trg_enforce_free_trial_teacher_limit
  BEFORE INSERT OR UPDATE OF role ON public.user_roles
  FOR EACH ROW EXECUTE FUNCTION public.enforce_free_trial_teacher_limit();
...
BEGIN
  ...
  INSERT INTO public.free_trial_members (trial_id, user_id, role, membership_kind)
  VALUES (v_trial_id, NEW.user_id, NEW.role::text, 'school_admin_invite')
  ON CONFLICT (trial_id, user_id) DO NOTHING;

  IF EXISTS (
    SELECT 1 FROM public.free_trial_members
    WHERE trial_id = v_trial_id AND user_id = NEW.user_id AND membership_kind = 'school_admin_invite'
  ) THEN
    INSERT INTO public.free_trial_events (trial_id, user_id, event_name)
    VALUES (v_trial_id, NEW.user_id, 'teacher_invited');
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER trg_track_free_trial_staff_member
  AFTER INSERT OR UPDATE OF role ON public.user_roles
  FOR EACH ROW EXECUTE FUNCTION public.track_free_trial_staff_member();
